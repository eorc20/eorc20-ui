...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.EVM]).toBeTruthy()
      expect(txs[ChainId.EVM]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.EVM]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.EVM_TESTNET]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.EVM), String(ChainId.EVM_TESTNET)])
      expect(Object.keys(store.getState()[ChainId.EVM] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.EVM_TESTNET] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.EVM }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.EVM), String(ChainId.EVM_TESTNET)])
      expect(Object.keys(store.getState()[ChainId.EVM] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.EVM_TESTNET] ?? {})).toEqual(['0x1'])
    })
  })
})
