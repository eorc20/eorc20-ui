...
import { ChainId } from '../swap-sdk/src/index'
...
export const multicallAddresses = {
  ...
  15557: '0x677031d4e244c9551279F46224BFA1695cb51BDB',
  15556: '0x677031d4e244c9551279F46224BFA1695cb51BDB',
  17777: '0x951eFabD8bcD2A9f65D1f2100fD9bf3afE96AA22',
}
...
export type MultiCallV22 = <T = any>(params: MulticallV22Params) => Promise<T>
...
export function createMulticall<TProvider>(provider: ({ chainId }: { chainId?: number | undefined }) => TProvider) {
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.EVM) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.EVM_NET, options }) => {
  ...
  const multicallv22: MultiCallV22 = async ({ abi, calls, chainId = ChainId.EVM_TESTNET, signer, options }) => {
    const { requireSuccess = true, ...overrides } = options || {}
    const multi = getMulticallContract(chainId, signer ?? provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
    const itf = new Interface(abi)
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.EVM_NET, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
    ...
    const result = await multi.aggregate3([...[_calls], ...(overrides ? [overrides] : [])])
