...
const WrongNetworkSelect = ({ switchNetwork, chainId }) => {
  ...
  const localChainId = useLocalNetworkChain() || ChainId.EVM_NET
  const [, setSessionChainId] = useSessionChainId()

  const localChainName = chains.find((c) => c.id === localChainId)?.name ?? 'EVM'
  ...
  return (
    <>
      ...
      <Box px="16px" pt="8px">
        <ArrowUpIcon color="text" /> : <ArrowDownIcon color="text" />
      </Box>
...
export const NetworkSwitcher = () => {
  ...
  // if (!chainId || chainId === ChainId.EVM_TESTNET) {
  //   return null
  // }
