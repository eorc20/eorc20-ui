...
import { useIsomorphicEffect } from "@inscription/uikit/src/hooks/useIsomorphicEffect";
import { breakpointMap } from "@inscription/uikit/src/theme/base";
...
export const getBreakpointChecks = (state: State): BreakpointChecks => {
  return {
    ...
    isDesktop: state.isXl || state.isXxl || state.isXxxl || state.isXxxxl,
  };
};
...
export const MatchBreakpointsProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  useIsomorphicEffect(() => {
    // Create listeners for each media query returning a function to unsuevmribe
