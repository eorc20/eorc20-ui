...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://public.pro - check the URL carefully.")
    return warningText.split(/(https:\/\/public.pro)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://public.pro'}
          color={text === 'https://public.pro' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
